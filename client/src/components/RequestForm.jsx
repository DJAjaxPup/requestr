...
export default function RequestForm({ onSubmit, disabled = false, connecting = false }) {
  ...
  const submit = (e) => {
    ...
    // parent will socket.emit with ACK and call our callback
    onSubmit(
      {
        song: s,
        artist: artist.trim(),
    ...
    // optimistic fallback so UI doesn’t get stuck if ACK is slow/missed
    setTimeout(() => {
      finish(true);
      // if this fires frequently, check server ACKs/logs
    }, 1800);
  };
